...
public class SettleAccountJobCmdServiceImpl implements SettleAccountJobCmdService {
    ...
    @Resource
    SettleAccountSnapshotService settleAccountSnapshotService;
    @Resource
    SettleAccountFlowService settleAccountFlowService;
    @Resource
    ...
    public void accountDailySnapshot(SettleAccountSnapshotJobCommand command) {
        ...
        settleAccountSnapshotService.remove(deleteQuery);

        //生成余额快照
        List<SettleAccount> accountList = settleAccountService.list();
        ...
        boolean saveBatch = settleAccountSnapshotService.saveBatch(snapshotList);
    ...
    public void accountFlowRevision(SettleAccountFlowRevisionJobCommand command) {
        ...
        flowQuery.lambda()
                ...
                .last(LIMIT_1);
        SettleAccountFlow accountFlow = settleAccountFlowService.getOne(flowQuery);
        Long flowId = 1L;
        ...
        List<SettleAccountFlow> groupAccountFlowList = settleAccountFlowService.list(flowGroupQuery);
    ...
    private void handlerFlowRevision(List<SettleAccountFlow> groupAccountFlowList, SettleAccount account, SettleAccountFlowRevisionJobCommand command) {
        ...
        if (balanceAmount.compareTo(flowSumAmount) != 0) {
            ...
            rocketMqProducer.syncSendJobMessage("余额&流水校对", "Exceptional", msg);
        }
    }
